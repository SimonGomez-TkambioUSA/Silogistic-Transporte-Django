...
module.exports = function(grunt) {
	...
	grunt.initConfig({
		...
		clean: {
			...
			post: ['components/tmp/', 'components/**/*.min.*']
		},
		copy: {
			jquery: {
				src: 'node_modules/jquery/components/jquery.js',
				dest: 'components/dependencies/jquery.js'
			},
			respond: {
				src: 'node_modules/respond.js/dest/respond.src.js',
				dest: 'components/dependencies/respond.js'
			},
			qunit: {
				files: [{
					expand: true,
		...
		concat: {
			...
			jsautoinit: {
				...
				dest: 'components/<%= pkg.name %>-init.js'
			},
			jsall: {
				src: [
					'src/tables.js',
			...
			jsstack: {
				...
				dest: 'components/stackonly/<%= pkg.name %>.stackonly.js'
			},
			cssall: {
				src: [
					'src/tables.css',
				...
				dest: 'components/tmp/<%= pkg.name %>.myth.css'
			},
			cssbare: {
				src: [
					'src/tables.css',
				...
				dest: 'components/tmp/<%= pkg.name %>.bare.myth.css'
			},
			cssstack: {
				src: [
					'src/tables.css',
				...
				dest: 'components/tmp/<%= pkg.name %>.stackonly.myth.css'
			},
			cssstackmixinpre: {
				src: [
					'src/tables.css',
				...
				dest: 'components/tmp/<%= pkg.name %>.stackonly.myth.scss'
			},
			cssstackmixinpost: {
				src: [
				...
				dest: 'components/stackonly/<%= pkg.name %>.stackonly.scss'
			}
		},
		qunit: {
			files: ['test/**/*.html']
		...
		uglify: {
			js: {
				files: {
					'dist/<%= pkg.name %>.min.js': [ 'components/<%= pkg.name %>.js' ],
					'dist/stackonly/<%= pkg.name %>.stackonly.min.js': [ 'components/stackonly/<%= pkg.name %>.stackonly.js' ]
				}
			}
		},
		cssmin: {
		...
		compress: {
			main: {
				options: {
					archive: 'components/tablesaw-<%= pkg.version %>.zip',
					mode: 'zip',
					pretty: true
				},
				files: [
					{expand: true, cwd: 'components/', src: ['*'], dest: 'tablesaw/'},
					{expand: true, cwd: 'components/', src: ['dependencies/*'], dest: 'tablesaw/'},
					{expand: true, cwd: 'components/', src: ['stackonly/*'], dest: 'tablesaw/'},
					{expand: true, cwd: 'components/', src: ['bare/*'], dest: 'tablesaw/'}
				]
			}
		}
	});
