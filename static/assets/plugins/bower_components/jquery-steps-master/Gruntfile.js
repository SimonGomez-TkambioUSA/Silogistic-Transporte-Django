...
module.exports = function (grunt)
{
    ...
    grunt.initConfig({
        ...
        exec: {
            createPkg: {
                cmd: "<%= pkg.folders.nuget %>\\Nuget pack <%= pkg.folders.nuget %>\\jQuery.Steps.nuspec -OutputDirectory <%= pkg.folders.components %> -Version <%= pkg.version %>"
            }
        },
        compress: {
            main: {
                options: {
                    archive: '<%= pkg.folders.components %>/jquery.steps-<%= pkg.version %>.zip'
                },
                files: [
                  { flatten: true, expand: true, src: ['<%= pkg.folders.components %>/*.js'], dest: '/' }
                ]
            }
        },
        uglify: {
