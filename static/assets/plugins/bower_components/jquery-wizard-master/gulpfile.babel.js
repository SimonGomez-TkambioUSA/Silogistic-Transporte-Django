...
// Generates compiled CSS and JS files and puts them in the components/ folder
gulp.task('deploy:components', gulp.series('build'));
gulp.task('deploy:prepare', gulp.series('deploy:prompt', 'deploy:version', 'deploy:init', 'deploy:components'));
gulp.task('deploy', gulp.series('deploy:prompt', 'deploy:version', 'deploy:message', 'deploy:components', 'deploy:commit'));
